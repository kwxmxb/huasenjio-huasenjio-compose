 ...
 * @LastEditTime: 2022-12-21 22:11:27
 * @Description: 
-->
...
<template>
  <div class="take">
    ...
    <main>
      ...
      <Draggable v-model="series" ghostClass="ghost" filter=".no-drap" chosenClass="chosen" animation="400" @end="onDragEnd">
        <transition-group>
          <div class="tbody" v-for="(typeItem, index) in series" :key="`${typeItem.typeName}${index}`">
            <div class="banner">
              ...
              <ul class="option no-drap">
                ...
                <li title="剪贴板粘贴" class="iconfont icon-wenzi text-yellow-500" @click="pasteSiteFromClipboard(index)"></li>
              </ul>
            </div>
            <div class="content no-drap">
...
export default {
  ...
  methods: {
    ...
    copySite(typeIndex, siteIndex) {
      // 拷贝到内存
      let target = this.series[typeIndex].sites[siteIndex];
      this.currentCopySite = this.TOOL.copyObject(target);
      this.$tips('success', '你复制已成功·粘贴至栏目中', 'top-right', 1200);

      // 同时拷贝到剪贴板
      this.TOOL.copyTextToClip(JSON.stringify(target), '你复制已成功·粘贴至栏目中', true);
    },
    ...
    // 从剪贴板粘贴链接
    async pasteSiteFromClipboard(typeIndex) {
      try {
        let rawData;
        ...
        this.series[typeIndex].sites.push(site);
        this.updateTake();
      } catch (err) {
        this.$tips('error', '剪贴板粘贴失败', 'top-right', 2000);
      }
    },
