 ...
 * @LastEditTime: 2023-02-03 22:33:22
 * @Description: 仪表盘页面
-->
<template>
  <div class="home">
    ...
    <el-tabs class="tab-group" type="border-card">
      ...
      <el-tab-pane>
        ...
        <div class="w-full h-px-400">
          <HomeSheet :tableData="dubious" :tableMap="dubiousMap"></HomeSheet>
        </div>
      </el-tab-pane>
      <el-tab-pane>
        <span slot="label">
...
export default {
  ...
  methods: {
    ...
    handleVisitorStatus(data) {
      ...
      let { ipMap, referers, hosts, dubious, traces } = data;
      // 处理ip集合
      ...
      // 处理可疑的请求
      dubious.map(log => {
        let exit = this.dubious.some(r => r.uid === log.uid);
        if (!exit) {
          this.dubious.push({
            ...log,
          });
